...
class Generator(DistributionMatchingGenerator):
    def __init__(self, model_name: str, gpu: int, ckpt: str):
        super().__init__()
        elif model_name == 'vae_piece':
            module = pl_models.vae_piece_model.VAEPieceModel
        else:
        ...
        model = module.load_from_checkpoint(ckpt)
        model.to(loc)
        self.model = model
        self.device = loc
...
def parse():
    ...
    parser.add_argument('--model', type=str, required=True, choices=['vae_piece', 'vae_dgmg', 'jtvae', 'graphaf', 'gcpn'],
                        help='Type of model')
...
def main(args):
    ...
    if os.path.exists(smiles_file):
        ...
        if args.model == 'graphaf':
            generator = GraphAFGenerator(args.dist_file, args.gpu, args.ckpt)
        elif args.model == 'gcpn':
            generator = GCPNGenerator(args.dist_file, args.gpu, args.ckpt)
        else:
            generator = Generator(args.model, args.gpu, args.ckpt)
